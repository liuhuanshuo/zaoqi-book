    ...
    var checkTocScroll = function() {
        ...
        margin_content.each((index, item) => {
            ...
            if (removeToc && window.pageYOffset > 20) {
                ...
                return false
            } else { $("div.bd-toc").addClass("show") };
        })
    };
    var manageScrolledClassOnBody = function() { if (window.scrollY > 0) { document.body.classList.add("scrolled"); } else { document.body.classList.remove("scrolled"); } }
    $(window).on('scroll', function() { if (!scrollTimeout) { scrollTimeout = setTimeout(function() { checkTocScroll();
                manageScrolledClassOnBody();
                scrollTimeout = null; }, throttle); } });
}
var initThebeSBT = () => {
